...
export default function Header() {
  ...
  const links = useMemo(
    () => [
      ...
      { href: "/#faqs", label: "FAQs", match: () => false },      // anchor on Home
      { href: "/#areas", label: "Areas we serve", match: () => false }, // anchor on Home
  ...
  // styling helpers
  const baseLink = "relative py-2 text-sm text-slate-300 hover:text-cyan-300 transition-colors";
  const underline = "absolute left-0 -bottom-1 h-[2px] w-full scale-x-0 group-hover:scale-x-100 origin-left transition-transform";
  ...
  return (
    <header
      ...
      `}
    >
      {/* subtle gradient strip for contrast */}
        ...
        <nav className="hidden xl:flex items-center gap-6">
          {links.map((l) => {
            ...
            return (
              ...
              >
                {l.label}
                <span
                  className={`${underline} ${activeUnderline} ${isActive ? "scale-x-100" : ""}`}
                />
              </Link>
            );
          })}
          ...
          <Link
            ...
            className="rounded-lg px-4 py-2 text-sm font-semibold
                       ...
                       hover:shadow-[0_0_24px_-6px_rgba(34,211,238,0.45)]
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm xl:hidden" onClick={() => setOpen(false)}>
          <div
            className="ml-auto h-full w-[78%] max-w-xs bg-[#0e1628] border-l border-white/10 p-6
            ...
            <div className="mt-6 grid gap-3">
              ...
              <Link
                ...
                className="mt-2 rounded-lg px-4 py-3 text-sm font-semibold
                           border border-cyan-300/30 text-cyan-300
                           bg-cyan-400/10 hover:bg-cyan-400/20 transition"
              >
                Get Quote
              </Link>
            </div>
