...
export default function PricingRoi() {
  ...
  const estMonthly =
    billing === "monthly" ? price.total : Math.round(price.total / 12);
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 md:p-8">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          {/* keep H3 (page already has H2 above this component) */}
          <h3 className="text-xl font-semibold">Managed IT plan & simple add-ons</h3>
        </div>

        {/* billing toggle (a11y safe) */}
        <div className="flex items-center gap-2" aria-label="Billing frequency">
          <span className={`text-sm ${billing === "monthly" ? "text-cyan-300" : "text-slate-400"}`}>
            Monthly
          </span>
          ...
          <button
            ...
            className="relative w-12 h-6 rounded-full bg-slate-700/70 border border-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/40"
          >
            <span
        ...
        {[
          ...
          >
            ...
            <div className="mt-2 text-2xl font-extrabold">
              {typeof p === "number" ? `USD ${p.toLocaleString()}` : p}{" "}
              {u && <span className="text-sm font-medium text-slate-300">/ {u}</span>}
            </div>

            <ul className="mt-3 space-y-1 text-sm text-slate-200">
              {list.map((x) => (
                <li key={x} className="flex gap-2">
                  <CheckCircle2 className="h-4 w-4 text-cyan-300" aria-hidden="true" /> {x}
                </li>
              ))}
            </ul>
            ...
            <Link
              href="/get-quote"
              className="mt-4 inline-flex items-center gap-2 text-sm rounded-lg px-3 py-2 border border-white/10 bg-white/5 hover:bg-white/10"
              aria-label={`Get a quote for ${t}`}
            >
      ...
      <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/40 p-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <label htmlFor={rangeId} className="text-sm text-slate-300">
              Users:
            </label>
            <input
              id={rangeId}
              ...
              aria-describedby={`${rangeId}-val`}
            />
            <output id={`${rangeId}-val`} className="text-sm font-medium">
              {seats}
            </output>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-3 mt-3 text-sm">
          <Pill>
            ...
            Est. monthly spend:
            <strong className="ms-1">USD {estMonthly.toLocaleString()}</strong>
          </Pill>

          <Pill>
            <Lock className="h-4 w-4 text-cyan-300" aria-hidden="true" />
            Security lift: <strong className="ms-1">EDR + MFA baseline</strong>
          </Pill>

          <Pill>
            <LineChart className="h-4 w-4 text-cyan-300" aria-hidden="true" />
            Productivity: <strong className="ms-1">↓ P1 to ≤15 min</strong>
          </Pill>
        </div>
      </div>
    </div>
