// app/services/projects-consulting/ClientPage.jsx
'use client';

import { motion } from 'framer-motion';
import Image from 'next/image';
import { Wrench, Server, Cloud, Cpu, BarChart3, Sparkles } from 'lucide-react';
    ...
    <main className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10">
        <motion.div className="absolute inset-0 opacity-70" initial={{ opacity: 0 }} animate={{ opacity: 0.7 }} transition={{ duration: 0.8 }}>
          <Image src="/images/services/projects-hero.svg" alt="Projects & Consulting" fill sizes="100vw" priority className="object-cover" />
        </motion.div>
        <div className="relative p-10 md:p-16 bg-gradient-to-t from-black/60 to-black/20">
          ...
          <motion.p className="mt-3 max-w-2xl text-lg text-slate-200" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.05 }}>
            Fixed-scope projects with clear timelines, acceptance criteria, and knowledge transfer.
          </motion.p>
      ...
      <Section title="What’s Included" subtitle="Everything you need, nothing you don’t.">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          <FeatureCard icon={Wrench} title="Network Refresh" desc="Modernize switching, Wi-Fi heatmaps, and segmentation." />
          <FeatureCard icon={Server} title="Server Decommission" desc="Exit legacy hardware with safe data migration and backups." />
          <FeatureCard icon={Cloud} title="Email/Domain Moves" desc="Cutovers, tenant-to-tenant, or rebrand domain projects." />
          <FeatureCard icon={Cpu} title="Directory Cleanup" desc="AD/Entra restructuring, GPO/policy refactor." />
          <FeatureCard icon={BarChart3} title="Cost Optimization" desc="Licensing and vendor consolidation for predictable spend." />
          <FeatureCard icon={Sparkles} title="Automation" desc="Scripting and low-code workflows for repetitive tasks." />
        </div>
      </Section>
      ...
      <Section title="Delivery Process" subtitle="Clear steps, consistent outcomes.">
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <Step n={1} title="Discovery" desc="Workshops and current-state docs." />
            <Step n={2} title="Plan" desc="Design, dependencies, and runbook + rollback." />
            <Step n={3} title="Implement" desc="Change windows and stakeholder comms." />
            <Step n={4} title="Handover" desc="As-built docs, training, and warranty." />
          </div>
          <FadeIn delay={0.1}>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 h-full">
              ...
              <ul className="list-disc list-inside text-slate-300 space-y-1">
                <li>As-built documentation</li><li>Rollback/runbooks</li><li>Knowledge transfer</li><li>Warranty details</li>
              </ul>
              <CTA />
            </div>
          </FadeIn>
      ...
      <Section title="FAQs">
        <div className="grid md:grid-cols-2 gap-4">
          <FAQ q="After-hours work?" a="Yes—evenings/weekends change windows available." />
          <FAQ q="Fixed-bid?" a="Yes—fixed or T&M depending on scope." />
          <FAQ q="Co-delivery?" a="We can staff alongside your IT or MSP." />
          <FAQ q="Warranties?" a="We pass vendor warranties and document configs for support." />
        </div>
      </Section>
    </main>
  );
